...
## 开场白
对故事不感兴趣的可以跳过这一段，emmm，好像也不是太长的样子，还以为会写很长呢。  

>甲用Excel做了一个数据库文档，做完之后感觉良好。  
>乙看了之后，感觉做成json格式的似乎更好一些，于是改了改。  
>丙看了json之后发现，这个不是和那个Vue的表单组件需要的json很像吗？于是又改了改。  
>丁看到这些后感觉既然前端都可以这么做了，那么后端是不是也可以……于是也改了改。  
>甲乙丙丁看到最后的效果后非常满意，这下终于不用996了！于是向老板请功。  
...
## 思路
由数据库文档出发，引出各种json，利用引擎、组件等实现基本的增删改查的功能！基本思路就是下面这个脑图。
...
### 数据库文档
做一个项目，数据库文档是一个最基本的文档了，其他的似乎可以省略，但是数据库文档是必选项！专业的数据库设计应该采用PowerDesigner，只是他设计的数据库文档，不方便用程序读取，所以这里采用了Excel的形式。  
Excel便于操作和使用，也可以方便的使用代码来读取内容，这样就灵活多了。比如下面这个样子。
【Excel格式的数据库文档】
![excel格式的数据库文档](image/datadoc01Excel.png)

### 页面效果
![数据库文档的列表](image/datadoc02excellist.png)
...
### 生成表单组件需要的json
可以做一个简单的函数，把文档的json变成组件需要的json，只需要设置几个主要属性即可，这样可以形成一个“清水房”的效果。可以先实现基本功能。然后在通过“精装修”的方法做细节调整。
```javascript
{
    "name":{
        "controlType":"101",
...
![表单展示](image/datadoc03jsons.png)
精装修，设置详细属性
![表单展示](image/datadoc04form.png)
